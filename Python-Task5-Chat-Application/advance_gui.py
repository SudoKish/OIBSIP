...
class AdvancedChatGUI:
    ...
    def join_room(self):
        ...
        if room_name == self.current_room:
            messagebox.showinfo(
                "Already in Room",
                f"You are already in #{room_name}."
            )
            return
    ...
    def process_server_message(self, message):
        ...
        if message.startswith("SYSTEM:"):
            ...
            system_message = message[7:]

            self.add_message(
                system_message
            )

            # Update current room after joining
            if " joined #" in system_message:

                room_part = system_message.split(
                    " joined #"
                )[1]

                room_name = room_part.rstrip(".")
                ...
                self.status_label.config(
                    text=f"Connected | Room: {room_name}"
                )

            return
        if message.startswith("ROOM_CREATED:"):

            room_name = message[13:]
        ...
        # Show desktop notification when the window
        # is not focused

        if self.root.state() != "normal" or not self.root.focus_displayof():
    ...
    def create_room(self):
        ...
        try:
            ...
            self.client_socket.send(
                f"/create {room_name}".encode("utf-8")
            )
    ...
    def send_message(self):
        ...
        message = convert_shortcodes(message)
